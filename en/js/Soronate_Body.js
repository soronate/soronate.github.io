	...
	const c=document.createElement("canvas");c.id="snow";document.body.appendChild(c);
	const x=c.getContext("2d");c.width=innerWidth;c.height=innerHeight;
	const n=120,f=Array.from({length:n},()=>({x:Math.random()*c.width,y:Math.random()*c.height,r:Math.random()*3+1,v:Math.random()*1.5+1}));
	(function d(){x.clearRect(0,0,c.width,c.height);x.fillStyle="white";
	f.forEach(a=>{x.beginPath();x.arc(a.x,a.y,a.r,0,Math.PI*2);x.fill();a.y+=a.v;if(a.y>c.height){a.y=-5;a.x=Math.random()*c.width}});
